...
p_mk <- foreach(i = c("mod13q1", "myd13q1", "gimms"), .packages = lib,
                txt = c("a)", "b)", "c)")) %dopar% {
  ...
  p <- visMannKendall(rst = rst_ndvi, xlab = "", ylab = "",
                      ...
                      filename = paste0(ch_dir_outdata, i, "_mk001_0312.tif"), 
...
## statistics
fls_mk001 <- list.files(ch_dir_outdata, pattern = "mk001_0312.tif$", 
                        full.names = TRUE)
...
val_mk001_aqua_rel <- val_mk001_aqua_abs / ncell(rst_mk001[[4]])
...
names(ltys) <- c("GIMMS", "Terra", "Aqua")

p_dens <- ggplot() + 
  geom_line(aes(x = val_mk[[2]], y = ..count../sum(..count..), linetype = "Terra"), 
            stat = "density", lwd = .8) + 
  geom_line(aes(x = val_mk[[3]], y = ..count../sum(..count..), linetype = "Aqua"), 
            stat = "density", lwd = .8) + 
  geom_line(aes(x = val_mk[[1]], y = ..count../sum(..count..), linetype = "GIMMS"), 
            stat = "density", lwd = .8) + 
  geom_text(aes(x = -.675, y = .016), label = "d)", fontface = "bold", size = 6) + 
  scale_linetype_manual("", breaks = c("GIMMS", "Terra", "Aqua"), values = ltys) + 
...
ch_fls_fig <- paste0(ch_dir_outdata, "fig01__mannkendall.png")
...
vp_dens <- viewport(x = .52, y = 0.15, width = .458, height = .3, 
                    just = c("left", "bottom"))
